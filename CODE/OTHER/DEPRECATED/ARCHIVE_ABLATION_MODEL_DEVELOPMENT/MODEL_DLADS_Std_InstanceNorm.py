...
class Conv_PC_DLADS(nn.Module):
    def __init__(self, numIn, numOut, act):
        ...
        if act=='LeakyReLU': 
            ...
            negSlope, nonlinearity = 0.2, 'leaky_relu'
        elif act=='ReLU': 
            self.act = nn.ReLU(inplace=True)
            negSlope, nonlinearity = 0.0, 'relu'
        ...
        #self.norm0 = nn.InstanceNorm2d(numOut, affine=True)
        self.conv1 = nn.Conv2d(in_channels=numOut, out_channels=numOut, kernel_size=3, stride=1, padding='same', bias=useBias)
        #self.norm1 = nn.InstanceNorm2d(numOut, affine=True)
        nn.init.xavier_uniform_(self.conv0.weight, gain=1.0)
        nn.init.xavier_uniform_(self.conv1.weight, gain=1.0)
        #nn.init.kaiming_uniform_(self.conv0.weight, a=negSlope, nonlinearity=nonlinearity)
        #nn.init.kaiming_uniform_(self.conv1.weight, a=negSlope, nonlinearity=nonlinearity)

    def forward(self, data):
        data = self.act(self.conv0(data))
        #data = self.norm0(data)
        data = self.act(self.conv1(data))
...
class Conv_UP_DLADS(nn.Module):
    def __init__(self, numIn, numOut):
        ...
        nn.init.xavier_uniform_(self.conv0.weight, gain=1.0)
        #nn.init.kaiming_uniform_(self.conv0.weight, a=0.0, nonlinearity='relu')
...
class Model_DLADS(nn.Module):
    def __init__(self, numFilt, numChan):
        ...
        #nn.init.kaiming_uniform_(self.convFn0.weight, a=0.0, nonlinearity='relu')
    ...
    def forward(self, data):
        ...
        convDn0 = self.convDn0(convIn0)
        convDn1 = self.convDn1(convDn0)
        convDn2 = self.convDn2(convDn1)
        convDn3 = self.convDn3(convDn2)
        convUp3 = self.convUp3(convDn3, convDn2)
        convUp2 = self.convUp2(convUp3, convDn1)
        convUp1 = self.convUp1(convUp2, convDn0)
