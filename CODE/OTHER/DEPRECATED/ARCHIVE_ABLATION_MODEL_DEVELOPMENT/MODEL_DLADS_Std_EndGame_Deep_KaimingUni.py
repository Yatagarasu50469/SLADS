...
class Conv_PC_DLADS(nn.Module):
    def __init__(self, numIn, numOut, act):
        ...
        if act=='LeakyReLU': 
            ...
            nonlinearity, a = 'leaky_relu', 0.2
        elif act=='ReLU': 
            self.act = nn.ReLU(inplace=True)
            nonlinearity, a = 'relu', 0.0
        ...
        nn.init.kaiming_uniform_(self.conv0.weight, a=a, nonlinearity=nonlinearity)
        nn.init.kaiming_uniform_(self.conv0.weight, a=a, nonlinearity=nonlinearity)
...
class Conv_Dn_DLADS(nn.Module):
    def __init__(self, numIn, numOut):
        ...
        self.conv2 = Conv_PC_DLADS(numOut, numOut, 'LeakyReLU')
...
class Conv_Up_DLADS(nn.Module):
    def __init__(self, numIn, numOut):
        ...
        self.conv2 = Conv_PC_DLADS(numIn, numOut, 'ReLU')
        nn.init.kaiming_uniform_(self.conv0.weight, nonlinearity='linear')
...
class Model_DLADS(nn.Module):
    def __init__(self, numFilt, numChan):
        super().__init__()
        if standardizeInputData: self.normInput = Standardize()
        self.convIn0 = nn.Conv2d(in_channels=numChan, out_channels=numFilt, kernel_size=1, stride=1, bias=False)
        self.convIn1 = Conv_PC_DLADS(numFilt, numFilt, 'LeakyReLU')
        ...
        nn.init.kaiming_uniform_(self.convIn0.weight, nonlinearity='linear')
        nn.init.kaiming_uniform_(self.convFn0.weight, nonlinearity='linear')
    ...
    def forward(self, data):
        ...
        if standardizeInputData: data = self.normInput(data)
