...
class Conv_BF_DLADS(nn.Module):
    def __init__(self, in_channels, stride, sigma):
        ...
        self.padding = sigma*2
        
    def forward(self, data):
        data = F.interpolate(data, size=torch.Size([data.shape[2]+self.padding,data.shape[3]+self.padding]), mode='nearest-exact')
        return F.conv2d(data, self.filter, stride=self.stride, groups=self.in_channels)
