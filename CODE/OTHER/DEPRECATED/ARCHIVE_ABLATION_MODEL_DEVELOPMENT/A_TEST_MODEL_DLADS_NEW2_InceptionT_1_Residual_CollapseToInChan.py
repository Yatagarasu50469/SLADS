...
class Conv_PC_DLADS(nn.Module):
    def __init__(self, numIn, numOut, non_linearity):
        ...
        self.gateO = Conv_SC_DLADS(numOut*4, numIn, 1, 'same', non_linearity)
    ...
    def forward(self, data0):
        ...
        return data0+self.gateO(torch.cat([gateI, data1, gate1, data2], 1))
...
class Conv_UP_DLADS(nn.Module):
    def __init__(self, numIn, numOut, numChan):
        super().__init__()
        self.blur = Conv_BF_DLADS(numIn, stride=1)
        self.conv0 = Conv_SC_DLADS(numIn, numIn, 2, 'same', 'relu')
        self.conv1 = Conv_PC_DLADS(numIn+numChan, numOut, 'relu')
...
class Model_DLADS(nn.Module):
    def __init__(self, numFilt, numChan):
        ...
        self.convDn1 = Conv_DN_DLADS(numChan, numFilt)
        self.convUp1 = Conv_UP_DLADS(numChan, numFilt, numChan)
        self.convUp0 = Conv_UP_DLADS(numChan*2, numFilt, numChan)
        self.convFn0 = Conv_SC_DLADS(numChan*3, 1, 1, 'same', 'relu')
