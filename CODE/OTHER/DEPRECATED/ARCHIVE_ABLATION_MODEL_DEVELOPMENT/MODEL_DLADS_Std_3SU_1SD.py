...
class Conv_BF_UP_DLADS(nn.Module):
...
class Conv_BF_DN_DLADS(nn.Module):
    def __init__(self, in_channels, stride):
        super().__init__()
        self.in_channels = in_channels
        self.stride = stride
...
class Conv_DN_DLADS(nn.Module):
    def __init__(self, numIn, numOut):
        super().__init__()
        self.conv0 = nn.Sequential(*[nn.MaxPool2d(kernel_size=2, stride=1), Conv_BF_DN_DLADS(numIn, stride=2)])
        self.conv1 = Conv_PC_DLADS(numIn, numOut, 'LeakyReLU')
...
class Conv_UP_DLADS(nn.Module):
    def __init__(self, numIn, numOut):
        ...
        self.blur = Conv_BF_UP_DLADS(numIn, stride=1)
