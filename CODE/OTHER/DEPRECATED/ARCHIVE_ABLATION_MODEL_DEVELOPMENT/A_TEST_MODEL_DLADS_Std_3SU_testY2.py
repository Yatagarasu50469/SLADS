...
class Conv_SC_DLADS(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, padding, non_linearity):
        ...
        if non_linearity=='leaky_relu': self.act, negative_slope = nn.LeakyReLU(negSlope, inplace=True), negSlope
        ...
        elif non_linearity=='prelu': self.act, negative_slope, non_linearity = nn.PReLU(init=0.2), negSlope, 'leaky_relu'
...
class Conv_IN_DLADS(nn.Module):
    def __init__(self, numIn, numOut):
        ...
        self.conv0 = Conv_SC_DLADS(numIn, numOut, 1, 'same', inAct)
        self.conv1 = Conv_PC_DLADS(numOut, numOut, emDnAct)
...
class Conv_DN_DLADS(nn.Module):
    def __init__(self, numIn, numOut):
        ...
        self.pool0 = nn.Sequential(*[nn.MaxPool2d(kernel_size=2, stride=1), Conv_BF_DLADS(numOut, stride=2)])
...
class Conv_UP_DLADS(nn.Module):
    def __init__(self, numIn, numOut):
        ...
        self.conv2 = Conv_SC_DLADS(numIn, numOut, 1, 'same', emUpAct)
        self.conv3 = Conv_PC_DLADS(numOut, numOut, upAct)
...
class Conv_FN_DLADS(nn.Module):
    ...
    def forward(self, data):
        data = self.conv(data)
        if torch.max(data).item() <= 0: return data
        else: return self.act(data)
...
class Model_DLADS(nn.Module):
    def __init__(self, numFilt, numChan):
        ...
        self.convFn0 = Conv_FN_DLADS(numFilt, 1, 1, 'same', fnAct)
