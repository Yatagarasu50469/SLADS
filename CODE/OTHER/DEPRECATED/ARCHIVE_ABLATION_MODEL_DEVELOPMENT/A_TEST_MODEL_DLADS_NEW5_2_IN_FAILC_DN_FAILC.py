...
#Processing convolutional block for DLADS
class Conv_PC_TOP_Fail_C_DLADS(nn.Module):
    def __init__(self, numIn, numOut, non_linearity):
        super().__init__()
        self.gateI = Conv_SC_DLADS(numIn, numOut, 1, 'same', non_linearity)
        self.conv0 = Conv_SC_DLADS(numOut, numOut, 3, 'same', non_linearity)
...
#Processing convolutional block for DLADS
class Conv_PC_TOP_Old_J_DLADS(nn.Module):
    def __init__(self, numIn, numOut, non_linearity):
        super().__init__()
        self.gateI = Conv_SC_DLADS(numIn, numOut, 1, 'same', non_linearity)
        self.conv1 = Conv_SC_DLADS(numOut, numOut, 3, 'same', non_linearity)
        self.gate1 = Conv_SC_DLADS(numOut, numOut, 1, 'same', non_linearity)
        self.conv2 = Conv_SC_DLADS(numOut, numOut, 3, 'same', non_linearity)
        self.gateO = Conv_SC_DLADS(numIn+numOut*4, numOut, 1, 'same', non_linearity)
        
    def forward(self, data0):
        gateI = self.gateI(data0)
        data1 = self.conv1(gateI)
        gate1 = self.gate1(data1)
        data2 = self.conv2(gate1)
        
        return self.gateO(torch.cat([data0, gateI, data1, gate1, data2], 1))
...
class Conv_IN_DLADS(nn.Module):
    def __init__(self, numIn, numOut):
        ...
        self.conv0 = Conv_PC_TOP_Fail_C_DLADS(numIn, numOut, inAct)
...
class Conv_DN_DLADS(nn.Module):
    def __init__(self, numIn, numOut):
        ...
        self.conv0 = Conv_PC_TOP_Fail_C_DLADS(numIn, numOut, dnAct)
