...
class Conv_Ac_DLADS(nn.Module):
    def __init__(self, numIn, numOut, act):
        ...
        else: sys.exit('Error - Unexpected activation function specified')
        self.conv0 = nn.Conv2d(in_channels=numIn, out_channels=numOut, kernel_size=3, stride=1, padding=1, bias=False, padding_mode='reflect')
        self.conv1 = nn.Conv2d(in_channels=numOut, out_channels=numOut, kernel_size=3, stride=1, padding=1, bias=False, padding_mode='reflect')
...
class Conv_Dn_DLADS(nn.Module):
    def __init__(self, numIn, numOut):
        super().__init__()
        self.conv0 = nn.Sequential(*[nn.MaxPool2d(kernel_size=2, stride=1), BlurPool(numIn, filt_size=7, stride=2)])
        self.conv1 = nn.Conv2d(in_channels=numIn, out_channels=numOut, kernel_size=1, stride=1, bias=False)
        self.conv2 = Conv_Ac_DLADS(numOut, numOut, 'LeakyReLU')
        nn.init.normal_(self.conv1.weight, mean=0.0, std=0.02)
    
    def forward(self, data):
        data = self.conv0(data)
        data = self.conv1(data)
        return self.conv2(data)

#Upsampling convolutional block for DLADS
class Conv_Up_DLADS(nn.Module):
    def __init__(self, numIn, numOut):
        super().__init__()
        self.conv0 = nn.Conv2d(in_channels=numIn, out_channels=numOut, kernel_size=1, stride=1, bias=False)
        self.conv1 = BlurPool(numOut, filt_size=7, stride=1)
        self.conv2 = Conv_Ac_DLADS(numIn, numOut, 'ReLU')
        nn.init.normal_(self.conv0.weight, mean=0.0, std=0.02)
    
    def forward(self, data, skip):
        data = self.conv0(data)
        data = self.conv1(F.interpolate(data, size=skip.shape[2:], mode='nearest-exact'))
        return self.conv2(torch.cat([data, skip], 1) )
    
#DLADS model
class Model_DLADS(nn.Module):
    def __init__(self, numFilt, numChan):
        super().__init__()
        self.convIn0 = nn.Conv2d(in_channels=numChan, out_channels=numFilt, kernel_size=1, stride=1, bias=False)
        self.convIn1 = Conv_Ac_DLADS(numFilt, numFilt, 'LeakyReLU')
        ...
        self.convFn0 = nn.Conv2d(in_channels=numFilt, out_channels=1, kernel_size=1, stride=1, bias=False)
        
        nn.init.normal_(self.convIn0.weight, mean=0.0, std=0.02)
        nn.init.normal_(self.convFn0.weight, mean=0.0, std=0.02)
        
    def forward(self, data, padding):
    
        if padInputData: 
            batches, channels, height, width = data.shape
            padHeight, padWidth = (-(-height//16)*16)-height, (-(-width//16)*16)-width
            data = F.pad(data, (padWidth, 0, padHeight, 0), mode='constant')
            for batchIndex in range(batches):
                for chanIndex in range(channels):
                    data[batchIndex, chanIndex, :padHeight or 1, :padWidth or 1] = padding[batchIndex, chanIndex]
        
        convDn0 = self.convIn1(self.convIn0(data))
        convDn1 = self.convDn1(convDn0)
        convDn2 = self.convDn2(convDn1)
        convDn3 = self.convDn3(convDn2)
        convDn4 = self.convDn4(convDn3)
...
class DataPreprocessing_DLADS(Dataset):
    def __init__(self, inputs, labels, padding, device, augmentFlag):
        super().__init__()
        ...
        if storeOnDevice:
            ...
            self.data_Padding = [torch.from_numpy(item).float().to(device) for item in padding]
        else: 
    ...
    def __getitem__(self, index):
        if self.noAugmentFlag: data, label, padding = self.data_Inputs[index], self.data_Labels[index], self.data_Padding[index]
        else: data, label, padding = *torch.tensor_split(self.transform(self.data_Merged[index]), self.channelSplit, 0), self.data_Padding[index]
...
class DLADS:
    ...
    def loadData(self, trainingDatabase, validationDatabase):
        ...
        inputs_TRN, labels_TRN, padding_TRN = [], [], []
        for sample in tqdm(trainingDatabase, desc = 'Training Data Setup', leave=True, ascii=asciiFlag):
            inputStack, paddingStack = prepareInput(sample.squareChanReconImages, sample.squareMask, trainingSampleData[sample.sampleDataIndex].squareOpticalImage)
            for chanNum in range(0, len(sample.squareRDs)):
                ...
                padding_TRN.append(paddingStack[chanNum])
        data_TRN = DataPreprocessing_DLADS(inputs_TRN, labels_TRN, padding_TRN, self.device, augTrainData)
        ...
        if len(validationDatabase)<=0: 
            ...
            inputs_VAL, labels_VAL, padding_VAL = [], [], []
            self.inputs_Viz, self.labels_Viz, self.padding_Viz = [], [], []
            for i, sample in enumerate(tqdm(validationDatabase, desc = 'Validation Data Setup', leave=True, ascii=asciiFlag)):
                inputStack, paddingStack = prepareInput(sample.squareChanReconImages, sample.squareMask, validationSampleData[sample.sampleDataIndex].squareOpticalImage)
                if i in vizSampleIndices:
                    if storeOnDevice: 
                        self.inputs_Viz.append(torch.from_numpy(inputStack).float().to(self.device))
                        self.padding_Viz.append(torch.from_numpy(paddingStack).float().to(self.device))
                    else: 
                        self.inputs_Viz.append(torch.from_numpy(inputStack).float())
                        self.padding_Viz.append(torch.from_numpy(paddingStack).float())
                ...
                for chanNum in range(0, len(sample.squareRDs)):
                    ...
                    padding_VAL.append(paddingStack[chanNum])
            data_VAL = DataPreprocessing_DLADS(inputs_VAL, labels_VAL, padding_VAL, self.device, False)
    ...
    def visualizeTraining(self, epoch):
        ...
        if self.valFlag: 
            for vizSampleNum in range(0, self.numViz): 
                ...
                if not storeOnDevice: 
                    input = self.inputs_Viz[vizSampleNum].to(self.device)
                    padding = self.padding_Viz[vizSampleNum].to(self.device)
                else: 
                    input = self.inputs_Viz[vizSampleNum]
                    padding = self.padding_Viz[vizSampleNum]
                
                squareRD = self.labels_Viz[vizSampleNum]
                with torch.inference_mode(): squareERD = torch.mean(self.model(input, padding), 0).detach().cpu().numpy()[0]
    ...
    def computeLoss(self, data, label, padding, trainFlag=False):
        
        #Zero network gradients
        if trainFlag: self.model.zero_grad()
        
        #Move data to device if not already there
        if not storeOnDevice: data, label, padding = data.to(self.device), label.to(self.device), padding.to(self.device)
        
        #Compute loss
        loss = torch.mean(torch.abs(self.model(data, padding)-label))
    ...
    def train(self):
        ...
        for epoch in trainingBar:
            ...
            self.loss_TRN.append(np.mean([self.computeLoss(data, label, padding, True) for data, label, padding in tqdm(self.dataloader_TRN, total=self.numTRN, desc='TRN', leave=False, ascii=asciiFlag)]))
            
            #Compute losses over the validation dataset
            _ = self.model.train(False)
            if self.valFlag: 
                with torch.inference_mode(): 
                    self.loss_VAL.append(np.mean([self.computeLoss(data, label, padding, False) for data, label, padding in tqdm(self.dataloader_VAL, total=self.numVAL, desc='VAL', leave=False, ascii=asciiFlag)]))
            
            #Update the current loss used for training status evaluation
            if self.valFlag: loss_CUR = self.loss_VAL[-1]
            ...
            if (loss_CUR < self.loss_BST): self.model_BST, self.opt_BST, self.rng_BST, self.loss_BST, self.epoch_BST, self.patienceCounter, self.stagnationCounter = copy.deepcopy(self.model.state_dict()), copy.deepcopy(self.opt.state_dict()), copy.deepcopy(torch.get_rng_state()), loss_CUR, epoch, 0, 0
