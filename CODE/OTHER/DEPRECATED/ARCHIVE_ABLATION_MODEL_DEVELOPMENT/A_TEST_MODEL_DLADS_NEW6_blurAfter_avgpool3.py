...
class Conv_DN_DLADS(nn.Module):
    def __init__(self, numIn, numOut):
        super().__init__()
        if sigmaDn == 0: self.down0 = nn.AvgPool2d(kernel_size=3, stride=2)
        else: self.down0 = nn.Sequential(*[nn.AvgPool2d(kernel_size=3, stride=1), Conv_BF_DLADS(numIn, 2, sigmaDn)])
