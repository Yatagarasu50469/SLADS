...
class Conv_SC_DLADS(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, padding, non_linearity):
        ...
        elif non_linearity=='prelu': self.act, negative_slope, non_linearity = nn.PReLU(init=preluSlope), preluSlope, 'leaky_relu'
...
class Conv_PC_DLADS(nn.Module):
    ...
    def forward(self, data0):
        data1 = self.conv0(data0)
        return self.conv1(data1)

#Processing convolutional block for DLADS
class Conv_PC_TOP_A_DLADS(nn.Module):
    def __init__(self, numIn, numOut, non_linearity):
        super().__init__()
        self.conv0 = Conv_SC_DLADS(numIn, numOut, 3, 'same', non_linearity)
        self.conv1 = Conv_SC_DLADS(numOut, numOut, 3, 'same', non_linearity)
        self.gateO = Conv_SC_DLADS(numOut*2, numOut, 1, 'same', non_linearity)
        
    def forward(self, data0):
        data1 = self.conv0(data0)
        data2 = self.conv1(data1)
        return self.gateO(torch.cat([data1, data2], 1))

#Processing convolutional block for DLADS
class Conv_PC_TOP_B_DLADS(nn.Module):
    def __init__(self, numIn, numOut, non_linearity):
        super().__init__()
        self.conv0 = Conv_SC_DLADS(numIn, numOut, 3, 'same', non_linearity)
        self.conv1 = Conv_SC_DLADS(numOut, numOut, 3, 'same', non_linearity)
        self.gateO = Conv_SC_DLADS(numIn+numOut*2, numOut, 1, 'same', non_linearity)
        
    def forward(self, data0):
        data1 = self.conv0(data0)
        data2 = self.conv1(data1)
        return self.gateO(torch.cat([data0, data1, data2], 1))

#Processing convolutional block for DLADS
class Conv_PC_TOP_C_DLADS(nn.Module):
    def __init__(self, numIn, numOut, non_linearity):
        super().__init__()
        self.gateI = Conv_SC_DLADS(numIn, numOut, 1, 'same', non_linearity)
        self.conv0 = Conv_SC_DLADS(numOut, numOut, 3, 'same', non_linearity)
        self.conv1 = Conv_SC_DLADS(numOut, numOut, 3, 'same', non_linearity)
        self.gateO = Conv_SC_DLADS(numOut*3, numOut, 1, 'same', non_linearity)
        
    def forward(self, data0):
        gateI = self.gateI(data0)
        data1 = self.conv0(gateI)
        data2 = self.conv1(data1)
        return self.gateO(torch.cat([gateI, data1, data2], 1))

#Processing convolutional block for DLADS
class Conv_PC_TOP_D_DLADS(nn.Module):
    def __init__(self, numIn, numOut, non_linearity):
        super().__init__()
        self.gateI = Conv_SC_DLADS(numIn, numOut, 1, 'same', non_linearity)
        self.conv0 = Conv_SC_DLADS(numOut, numOut, 3, 'same', non_linearity)
...
#Processing convolutional block for DLADS
class Conv_PC_TOP_E_DLADS(nn.Module):
    def __init__(self, numIn, numOut, non_linearity):
        super().__init__()
        self.gateF = Conv_SC_DLADS(numIn, numOut, 1, 'same', non_linearity)
        self.gateI = Conv_SC_DLADS(numOut, numOut, 1, 'same', non_linearity)
        self.conv0 = Conv_SC_DLADS(numOut, numOut, 3, 'same', non_linearity)
        self.conv1 = Conv_SC_DLADS(numOut, numOut, 3, 'same', non_linearity)
        self.gateO = Conv_SC_DLADS(numOut*4, numOut, 1, 'same', non_linearity)
        
    def forward(self, data0):
        gateF = self.gateF(data0)
        gateI = self.gateI(gateF)
        data1 = self.conv0(gateI)
        data2 = self.conv1(data1)
        return self.gateO(torch.cat([gateF, gateI, data1, data2], 1))

#Processing convolutional block for DLADS
class Conv_PC_TOP_J_DLADS(nn.Module):
    def __init__(self, numIn, numOut, non_linearity):
        super().__init__()
        self.gateF = Conv_SC_DLADS(numIn, numOut, 1, 'same', non_linearity)
        self.gateI = Conv_SC_DLADS(numOut, numOut, 1, 'same', non_linearity)
        self.conv0 = Conv_SC_DLADS(numOut, numOut, 3, 'same', non_linearity)
        self.gateB = Conv_SC_DLADS(numOut, numOut, 1, 'same', non_linearity)
        self.conv1 = Conv_SC_DLADS(numOut, numOut, 3, 'same', non_linearity)
        self.gateO = Conv_SC_DLADS(numOut*5, numOut, 1, 'same', non_linearity)
        
    def forward(self, data0):
        gateF = self.gateF(data0)
        gateI = self.gateI(gateF)
        data1 = self.conv0(gateI)
        gateB = self.gateB(data1)
        data2 = self.conv1(gateB)
        return self.gateO(torch.cat([gateF, gateI, data1, gateB, data2], 1))

#Processing convolutional block for DLADS
class Conv_PC_TOP_K_DLADS(nn.Module):
    def __init__(self, numIn, numOut, non_linearity):
        super().__init__()
        self.gateF = Conv_SC_DLADS(numIn, numIn, 1, 'same', non_linearity)
        self.gateI = Conv_SC_DLADS(numIn, numOut, 1, 'same', non_linearity)
        self.conv0 = Conv_SC_DLADS(numOut, numOut, 3, 'same', non_linearity)
        self.conv1 = Conv_SC_DLADS(numOut, numOut, 3, 'same', non_linearity)
        self.gateO = Conv_SC_DLADS(numIn+numOut*3, numOut, 1, 'same', non_linearity)
...
class Conv_IN_DLADS(nn.Module):
    def __init__(self, numIn, numOut):
        ...
        self.conv0 = Conv_PC_TOP_K_DLADS(numIn, numOut, inAct)
        
    def forward(self, data):
        data0 = self.std(data)
        return self.conv0(data0)
...
class Conv_DN_DLADS(nn.Module):
    def __init__(self, numIn, numOut):
        ...
        self.conv0 = Conv_PC_DLADS(numIn, numOut, dnAct)
    
    def forward(self, data):
        data = self.down0(data)
...
class Conv_UP_DLADS(nn.Module):
    def __init__(self, numIn, numOut):
        ...
        self.conv1 = Conv_PC_DLADS(numIn*2, numOut, upAct)
