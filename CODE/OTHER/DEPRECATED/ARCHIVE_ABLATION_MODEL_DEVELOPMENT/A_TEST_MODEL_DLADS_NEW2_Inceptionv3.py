...
class Conv_PC_DLADS(nn.Module):
    def __init__(self, numIn, numOut, non_linearity):
        super().__init__()
        self.gateA = Conv_SC_DLADS(numIn, numOut, 1, 'same', non_linearity)
        self.convA1 = Conv_SC_DLADS(numOut, numOut, 3, 'same', non_linearity)
        self.convA2 = Conv_SC_DLADS(numOut, numOut, 3, 'same', non_linearity)
        
        self.gateB = Conv_SC_DLADS(numIn, numOut, 1, 'same', non_linearity)
        self.convB1 = Conv_SC_DLADS(numOut, numOut, 3, 'same', non_linearity)
        
        self.gateC = Conv_SC_DLADS(numIn, numOut, 1, 'same', non_linearity)
        
        self.gateO = Conv_SC_DLADS(numOut*3, numOut, 1, 'same', non_linearity)
        
    def forward(self, data0):
        gateA = self.gateA(data0)
        dataA1 = self.convA1(gateA)
        dataA2 = self.convA2(dataA1)
        
        gateB = self.gateB(data0)
        dataB1 = self.convB1(gateB)
        
        gateC = self.gateC(data0)
        
        return self.gateO(torch.cat([dataA2, dataB1, gateC], 1))
...
class Conv_IN_DLADS(nn.Module):
    def __init__(self, numIn, numOut):
        ...
        self.conv0 = Conv_PC_DLADS(numIn, numOut, 'leaky_relu')
...
class Conv_UP_DLADS(nn.Module):
    def __init__(self, numIn, numOut):
        ...
        self.conv0 = Conv_SC_DLADS(numIn, numOut, 2, 'same', 'relu')
        self.conv1 = Conv_PC_DLADS(numIn*2, numOut, 'relu')
...
class Model_DLADS(nn.Module):
    def __init__(self, numFilt, numChan):
        ...
        self.convFn0 = Conv_SC_DLADS(numFilt, 1, 1, 'same', 'relu')
