...
class Conv_PC_DLADS(nn.Module):
    def __init__(self, numIn, numOut, non_linearity):
        ...
        #self.gate3 = Conv_SC_DLADS(numOut, numOut, 1, 'same', non_linearity)
        
        self.gateO = Conv_SC_DLADS(numOut*3, numOut, 1, 'same', non_linearity)
        
    def forward(self, data):
        data0 = self.gateI(data)
        gateB = self.gateB(data0)
        
        data1 = self.conv1(data0)
        gate1 = self.gate1(data1)
        
        data2 = self.conv2(data1)
        gate2 = self.gate2(data2)
        
        #data3 = self.conv3(data2)
        #gate3 = self.gate3(data3)
        
        #return self.gateO(torch.cat([gate0, gate1, gate2, gate3], 1))
        return self.gateO(torch.cat([gateB, gate1, gate2], 1))
