...
class Conv_PC_DLADS(nn.Module):
    def __init__(self, numIn, numOut, non_linearity):
        ...
        self.gate2 = Conv_SC_DLADS(numOut*4, numOut, 1, 'same', non_linearity)
        
    def forward(self, data):
        gate0 = self.gate0(data)
        gate1 = self.gate1(data)
        data1 = self.conv1(gate0)
        data2 = self.conv2(data1)
        #data3 = self.conv3(data2)
        #return self.gate1(torch.cat([gate0, data1, data2, data3], 1))
        return self.gate2(torch.cat([gate1, gate0, data1, data2], 1))
