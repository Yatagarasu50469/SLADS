...
def computeRDPPs(labels, recons):
    if standardizeInputData: 
        ...
        meanValue, varValue = torch.mean(paddedRecons, axis=(-1, -2)).squeeze(), torch.var(paddedRecons, axis=(-1, -2), correction=0).squeeze()
        recons = F.instance_norm(recons[None,::], running_mean=meanValue, running_var=varValue, use_input_stats=False)[0].numpy()
        labels = F.instance_norm(labels[None,::], running_mean=meanValue, running_var=varValue, use_input_stats=False)[0].numpy()
...
class Conv_Ac_DLADS(nn.Module):
    def __init__(self, numIn, numOut, act):
        ...
        self.conv0 = nn.Conv2d(in_channels=numIn, out_channels=numOut, kernel_size=3, stride=1, padding=1, bias=False)
        self.conv1 = nn.Conv2d(in_channels=numOut, out_channels=numOut, kernel_size=3, stride=1, padding=1, bias=False)
...
class Conv_Dn_DLADS(nn.Module):
    def __init__(self, numIn, numOut):
        super().__init__()
        self.conv0 = nn.Sequential(*[nn.MaxPool2d(kernel_size=2, stride=1), Conv_BF_DLADS(numIn, stride=2)])
...
class Conv_Up_DLADS(nn.Module):
    def __init__(self, numIn, numOut):
        ...
        self.conv1 = Conv_BF_DLADS(numOut, stride=1)
...
class Model_DLADS(nn.Module):
    def __init__(self, numFilt, numChan):
        ...
        self.norm0 = nn.InstanceNorm2d(numChan)
    ...
    def forward(self, data):
        ...
        if standardizeInputData: data = self.norm0(data)
