...
class Conv_PC_DLADS(nn.Module):
    def __init__(self, numIn, numOut, non_linearity):
        ...
        self.gate1 = Conv_SC_DLADS(numIn, numOut, 1, 'same', non_linearity)
    ...
    def forward(self, data0):
        gate1 = self.gate1(data0)
        data1 = self.conv1(gate1)
        gate2 = self.gate2(data1)
        data2 = self.conv2(gate2)
        
        return self.gateI(data0)+self.gateO(torch.cat([gate1, data1, gate2, data2], 1))
