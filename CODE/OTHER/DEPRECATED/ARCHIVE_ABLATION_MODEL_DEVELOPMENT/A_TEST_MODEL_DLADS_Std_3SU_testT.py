...
class DLADS:
    ...
    def computeLoss(self, data, label, trainFlag=False, epoch=None, step=None):
        ...
        #Set random learning rate in specified range
        if trainFlag: 
            LR_New = torch.FloatTensor(1).uniform_(1e-7, learningRate).item()
            for paramGroup in self.opt.param_groups: paramGroup['lr'] = LR_New
