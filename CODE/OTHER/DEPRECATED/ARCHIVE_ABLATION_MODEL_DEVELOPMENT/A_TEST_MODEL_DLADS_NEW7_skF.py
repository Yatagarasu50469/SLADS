...
class Conv_BF_DLADS(nn.Module):
    def __init__(self, in_channels, stride, sigma):
        ...
        if blurPaddingMode == 'reflection': self.pad = PadRef(sigma)
        elif blurPaddingMode == 'zero': elf.pad = nn.ZeroPad2d(sigma)
    ...
    def forward(self, data):
        ...
        if blurPaddingMode == 'partial':
            with torch.no_grad():
                ...
                self.mask_ratio = torch.nan_to_num(self.slide_winsize/update_mask)
                update_mask = torch.clamp(update_mask, 0, 1)
                self.mask_ratio = torch.mul(self.mask_ratio, update_mask)
            raw_out = F.conv2d(self.pad(data), self.filter, stride=self.stride, groups=self.in_channels)
            return torch.mul(raw_out, self.mask_ratio)
...
class Conv_SC_DLADS(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, padding, non_linearity):
        ...
        elif non_linearity=='leaky_relu_sk': self.act, negative_slope = nn.LeakyReLU(leakySlopeSk, inplace=True), leakySlopeSk
...
class Conv_UP_DLADS(nn.Module):
    def __init__(self, numIn, numOut):
        ...
        self.convS = Conv_SC_DLADS(numOut*2, numOut, 1, 'same', skAct)
        self.conv1 = Conv_CB_DLADS(numOut, numOut, upAct, False)
        
    def forward(self, data, skip):
        data = F.interpolate(data, size=skip.shape[2:], mode='nearest-exact')
        data = self.blur(self.conv0(data))
        data = self.convS(torch.cat([data, skip], 1))
