...
#Dimension reduction convolutional block for DLADS
class Conv_DR_DLADS(nn.Module):
    ...
    def forward(self, data):
        return self.act(self.conv0(data))
...
class Conv_PC_DLADS(nn.Module):
    def __init__(self, numIn, numOut, act):
        ...
        self.conv0 = nn.Conv2d(in_channels=numIn, out_channels=numOut, kernel_size=3, stride=1, padding='same', bias=useBias)
...
class Conv_DN_DLADS(nn.Module):
    def __init__(self, numIn, numOut):
        super().__init__()
        self.conv0 = Conv_DR_DLADS(numIn, numOut, 'LeakyReLU')
        self.pool = nn.Sequential(*[nn.MaxPool2d(kernel_size=2, stride=1), Conv_BF_DLADS(numOut, stride=2)])
        self.conv1 = Conv_PC_DLADS(numOut, numOut, 'LeakyReLU')
...
class Conv_UP_DLADS(nn.Module):
    def __init__(self, numIn, numOut):
        ...
        self.conv0 = Conv_DR_DLADS(numIn, numOut, 'ReLU')
        self.conv1 = Conv_PC_DLADS(numOut, numOut, 'ReLU')
        self.conv2 = Conv_DR_DLADS(numIn, numOut, 'ReLU')
        self.conv3 = Conv_PC_DLADS(numOut, numOut, 'ReLU')
        
    def forward(self, data, skip):
        data = self.blur(F.interpolate(data, size=skip.shape[2:], mode='nearest-exact'))
        data = self.conv0(data)
        data = self.conv1(data)
        data = self.conv2(torch.cat([data, skip], 1))
        return self.conv3(data)
...
class Model_DLADS(nn.Module):
    def __init__(self, numFilt, numChan):
        ...
        self.convIn0 = Conv_DR_DLADS(numChan, numFilt, 'LeakyReLU')
        ...
        self.convFn0 = Conv_DR_DLADS(numFilt, 1, 'ReLU')
...
class DLADS:
    ...
    def visualizeTraining(self, epoch):
        ...
        if cosineScheduler: plotTitle = 'EPOCH: ' + ('{:0'+str(maxEpochsPrecision)+'}').format(epoch)  + ', LR: ' + '{:.1e}'.format(self.scheduler.get_last_lr()[0])
        else: plotTitle = 'EPOCH: ' + ('{:0'+str(maxEpochsPrecision)+'}').format(epoch) + ', LR: ' + '{:.1e}'.format(self.learningRate)
    ...
    def computeLoss(self, data, label, trainFlag=False, epoch=None, step=None):
        ...
        if trainFlag:
            loss.backward()
            self.opt.step()
            if cosineScheduler: self.scheduler.step(epoch+step/self.numTRN)
    ...
    def train(self):
        ...
        #Setup learning rate, optimizer, and scheduler
        self.learningRate = copy.deepcopy(learningRate)
        self.setOptimizer()
        
        if cosineScheduler: self.scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(self.opt, schedPeriod, schedMult)
