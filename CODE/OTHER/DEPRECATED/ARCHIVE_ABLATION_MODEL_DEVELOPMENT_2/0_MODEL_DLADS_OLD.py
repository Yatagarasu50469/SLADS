...
def computeRDPPs(labels, recons):
    ...
    if processInputData != 'None':
        ...
        if padInputData: 
            padDivisor = (2**networkDepth)
            paddedRecons = F.pad(recons, ((-(-recons.shape[-1]//padDivisor)*padDivisor)-recons.shape[-1], 0, (-(-recons.shape[-2]//padDivisor)*padDivisor)-recons.shape[-2], 0), mode='constant')
...
class CustomNorm(nn.Module):
    ...
    def forward(self, data): 
        minValue = torch.min(data, axis=(-1, -2), keepdims=True)
        return torch.nan_to_num((data-minValue)/(torch.max(data, axis=(-1, -2), keepdims=True) - minValue))
