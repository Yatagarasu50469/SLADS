...
def simulateSampling(sortedSampleFolders, dir_Results, optimalC, modelName):
    ...
    if not benchmarkNoProcessing:
        ...
        #allAvgTimesFileLoad = np.concatenate(allAvgTimesFileLoad)
        #allAvgTimesComputeRecon = np.concatenate(allAvgTimesComputeRecon)
        #allAvgTimesComputeERD = np.concatenate(allAvgTimesComputeERD)
    ...
    dataPrintout.append(['Targeted Reconstruction Compute Time (s):', np.mean(allAvgTimesComputeRecon)])
    dataPrintout.append(['ERD Compute Time (s):', np.mean(allAvgTimesComputeERD)])
