...
def testSLADS(sortedTestingSampleFolders, bestC, bestModel):
    ...
    for sampleNum in tqdm(range(0,len(testingSamples)), desc='Testing Samples', position=0, leave=True, ascii=True):
        t0 = time.time()
        result = runSLADS(info, [testingSamples[sampleNum]], bestModel, scanMethod, stopPerc, 0, simulationFlag=True, trainPlotFlag=False, animationFlag=animationGen, tqdmHide=False, bestCFlag=False)
