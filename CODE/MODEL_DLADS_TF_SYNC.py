...
#MODEL: DLADS-TF-SYNC
#
#TensorFlow equivalent implementation to MODEL_DLADS_PY_SYNC.py  (provided configuration is set appropriately)
#
#NOTE(S): DIFFERENCES WITH RESPECT TO DLADS_TF_DEP.py
#
# Training option to reshuffle_each_iteration has been enabled; this has increased regularization and results in smoother convergence.
#
# Augmentation now only uses random rotations in 90 degree increments and horizonal/vertical flips.
#
...
def unet(numFilters, numChannels):
    ...
    outputs = Conv2D(1, 1, activation='relu', padding='same', use_bias=useBias)(conv8)
...
class UNet_Padded(tf.keras.Model):
    ...
    def __init__(self, numFilters, numChannels, train=True, modelRef=None):
        super().__init__()
        if train: self.subModel = unet(numFilters, numChannels)
        else: self.subModel = tf.function(tf.keras.models.load_model(modelRef, compile=False), experimental_relax_shapes=True)
...
class DLADS_TF_SYNC:
    def __init__(self, trainFlag, local_gpus, modelDirectory=None, modelName=None):
        ...
        with self.device:
            ...
            if not trainFlag: 
                with self.device: 
                    if padInputData: self.model = UNet_Padded(numStartFilters, len(inputChannels), train=False, modelRef=modelDirectory+modelName)
                    else: self.model = tf.function(tf.keras.models.load_model(modelDirectory+modelName, compile=False), experimental_relax_shapes=True)
    ...
    def setOptimizer(self):
        if optimizer == 'NAdam': self.opt = Nadam(learning_rate=self.learningRate, epsilon=1e-08)
