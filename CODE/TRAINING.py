...
def initTrain(sortedTrainingSampleFolders):
    ...
    for sampleNum in tqdm(range(0,len(sortedTrainingSampleFolders)), desc = 'Training Samples', position=0, ascii=True):
        ...
        for measurementPercNum in tqdm(range(0,len(measurementPercs)), desc = 'Measurement %', position=1, leave=False, ascii=True):
            ...
            idens = [parFunction.remote(np.asarray(images[imNum])[mask==1], neighborIndices_id, info_id, maskObject_id, unMeasuredIdxs_id, measuredIdxs_id, neighborWeights_id, neighborDistances_id, orderForRD_id) for imNum in range(0, len(images))]
            for result in tqdm(parIterator(idens), total=len(idens), position=2, desc='Poly Features', leave=False, ascii=True):
            ...
            for cNum in tqdm(range(0,len(cValues)), desc = 'c Values', position=2, leave=False, ascii=True):
                c, sigma = cValues[cNum], sigmaValues[cNum]

                #For each of the selected unmeasured points calculate the captured "area"
                for index in tqdm(range(0,len(orderForRD)), desc = 'Gaussian', leave = False, ascii=True):
...
def findBestC(trainingSamples, trainingModels):
    ...
    trainingModels_id = ray.put(trainingModels)
    ...
    for cNum in tqdm(range(0, len(cValues)), desc = 'c Values', position=0, leave=True, ascii=True):
        ...
        #Perform pool function and extract variables from the results
        idens = [parFunction.remote(info_id, trainingSamples_id, trainingModels_id, stopPerc_id, cNum, sampleNum, simulationFlag_id, trainPlotFlag_id, animationFlag_id, tqdmHide_id) for sampleNum in range(0, len(trainingSamples))]
