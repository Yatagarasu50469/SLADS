    ...
    for trainingSampleFolder in tqdm(sortedTrainingSampleFolders, desc = 'Training Samples', position=0, ascii=True):
        ...
        for measurementPercNum in tqdm(range(0,len(measurementPercs)), desc = 'Measurement %', position=1, leave=False, ascii=True):
            ...
            for maskNum in tqdm(range(0,numMasks), desc = 'Masks', position=2, leave=False, ascii=True):
                ...
                measuredValues = avgGroundTruthImage[np.asarray(idxsX), np.asarray(idxsY)]
                ...
                numRandChoices =  int((100-measurementPerc)*percOfRD*imageWidth*imageHeight/(100*100))
                orderForRD = random.sample(range(0,maskObject.unMeasuredIdxs.shape[0]), numRandChoices)
                ...
                polyFeatures = computeFeatures(maskObject, maskObject.unMeasuredIdxs, neighborValues, neighborWeights, neighborDistances, info, reconImage)[orderForRD,:]
                ...
                for result in tqdm(parIterator(idens), total=len(idens), position=3, desc='c Values', leave=False, ascii=True):
                    ...
                    RDImage[maskObject.unMeasuredIdxsList[measurementPercNum][maskNum][:,0][orderForRD], maskObject.unMeasuredIdxsList[measurementPercNum][maskNum][:,1][orderForRD]] = result[1]
...
def bestC_parhelper(info_id, trainingSamples_id, trainingModel_id, scanMethod_id, stopPerc_id, sampleNum, simulationFlag_id, trainPlotFlag_id, animationFlag_id, tqdmHide_id, bestCFlag_id):
    result = runSLADS(info_id, trainingSamples_id, trainingModel_id, scanMethod_id, stopPerc_id, sampleNum, simulationFlag_id, trainPlotFlag_id, animationFlag_id, tqdmHide_id, bestCFlag_id)
...
def findBestC(trainingSamples, trainingModels):
    ...
    scanMethod_id = ray.put('pointwise')
    ...
    for cNum in tqdm(range(0, len(cValues)), desc = 'c Values', position=0, leave=True, ascii=True):
        ...
        #for sampleNum in range(0, len(trainingSamples)): areaUnderCurve += bestC_parhelper(info, trainingSamples, trainingModels[cNum], 'pointwise', stopPerc, sampleNum, True, False, False, False, True)
        
        #Perform pool function and extract variables from the results
        idens = [parFunction.remote(info_id, trainingSamples_id, trainingModel_id, scanMethod_id, stopPerc_id, sampleNum, simulationFlag_id, trainPlotFlag_id, animationFlag_id, tqdmHide_id, bestCFlag_id) for sampleNum in range(0, len(trainingSamples))]
