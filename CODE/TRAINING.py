...
class TrainingSample:
    def __init__(self, name, avgGroundTruthImage, maskObject, massRanges, measurementPerc, maskNum, cValue, reconImage, measuredImage, RDImage):
        self.name = name
...
def epochCalculate(model, inputValidationTensors, outputValidationTensors, validationPadding, totalValidationPSNR):
    ...
    for index in range(0, len(inputValidationTensors)):
        ...
        #Remove network compatability padding
        dimWithoutPadding = np.asarray(pred_ERD.shape)-validationPadding[index]
        pred_ERD = pred_ERD[:dimWithoutPadding[0],:dimWithoutPadding[1]]
        RD = RD[:dimWithoutPadding[0],:dimWithoutPadding[1]]
...
def epochDisplay(epoch, trainingDatabase, inputValidationTensors, outputValidationTensors, validationPadding, totalValidationPSNR, totalTrainingLosses, totalValidationLosses, cValue, cNum, numTraining, patience, maxPatience, bestPSNR, bestEpoch, psnrList, avgPSNR, ERDImages):
    ...
    sampleLocations = [0, len(measurementPercs)]
    ...
    for rowNum in range(1, len(sampleLocations)+1):
        ...
        validationTensorPadding = validationPadding[sampleLocations[rowNum-1]]
        ...
        #Remove network compatability padding
        dimWithoutPadding = np.asarray(pred_ERD.shape)-validationTensorPadding
        pred_ERD = pred_ERD[:dimWithoutPadding[0],:dimWithoutPadding[1]]
        RD = RD[:dimWithoutPadding[0],:dimWithoutPadding[1]]
        measuredImage = measuredImage[:dimWithoutPadding[0],:dimWithoutPadding[1]]
    
        ax = plt.subplot2grid((3,3), (rowNum,0))
        ax.imshow(measuredImage, aspect='auto', cmap='hot')
        ax.set_title('Measured', fontsize=15, fontweight='bold') 

        ax = plt.subplot2grid((3,3), (rowNum,1))
        im = ax.imshow(RD, aspect='auto', vmin=0.0)
        ax.set_title('RD', fontsize=15, fontweight='bold')
        cbar = f.colorbar(im, ax=ax, orientation='vertical', pad=0.01)

        ax = plt.subplot2grid((3,3), (rowNum,2))
...
def generateDataset(dataset, RDValues, percValues, sample, avgGroundTruthImage, percToScan, massRanges, dir_TrainingResultsImages, dataSampleName, maskNum, cValue, imageWidth, imageHeight, incrementalFlag):
    ...
    for measurementPerc in tqdm([initialPercToScan]+measurementPercs, desc = '%', leave=False, ascii=True):

        #Until the next measurement percetnage has been reached, continue scanning
        while (round(percMeasured) < measurementPerc):
        #or ((scanMethod == 'linewise') and (len(maskObject.linesToScan) == 0) and not lineRevistFlag):, inconsistent number of lines 

            #If this is the first iteration, then create a new mask object (force pointwise initialization) and set initial avgImage
            if measurementPerc == initialPercToScan:
                maskObject = MaskObject(imageWidth, imageHeight, initialPercToScan, 'pointwise')
                sample.avgImage = avgGroundTruthImage*maskObject.mask
            else:
                if incrementalFlag: 
                    maskObject, newIdxs = findNewMeasurementIdxs(maskObject, sample, RDImage, percToScan, False)
                else:
                    maskObject, newIdxs = findNewMeasurementIdxs(maskObject, sample, RDImage, percToScan, True)

                #Perform measurements, updating sample/maskObject accordingly
            ...
            dataset.append(TrainingSample(sample.name, avgGroundTruthImage, copy.deepcopy(maskObject), massRanges, percMeasured, maskNum, cValue, reconImage, measuredImage, RDImage))
            
            #Visualize and save
            if incrementalFlag: 
                saveLocation = dir_TrainingResultsImages+ 'training_incr_'+ 'c_' + str(cValue) + '_var_' + str(maskNum) + '_' + sample.name + '_perc_' + str(round(percMeasured, 4))+ '.png'
            else:
                saveLocation = dir_TrainingResultsImages+ 'training_rand_'+ 'c_' + str(cValue) + '_var_' + str(maskNum) + '_' + sample.name + '_perc_' + str(round(percMeasured, 4))+ '.png'
            ...
            RDValues.append(reconPSNR)
            percValues.append(percMeasured)
            ...
            if incrementalFlag:
                plt.savefig(dir_TrainingResultsImages + 'incr_c_' + str(cValue) + '_mask_'+ sample.name + '_percentage_' + str(round(percMeasured, 4)) + '_variation_' + str(maskNum) + '.png', bbox_inches=extent)
            else:
                plt.savefig(dir_TrainingResultsImages + 'rand_c_' + str(cValue) + '_mask_'+ sample.name + '_percentage_' + str(round(percMeasured, 4)) + '_variation_' + str(maskNum) + '.png', bbox_inches=extent)
            plt.close()
            ...
            extent = ax.get_window_extent().transformed(fig.dpi_scale_trans.inverted())
            if incrementalFlag:
                plt.savefig(dir_TrainingResultsImages + 'incr_c_' + str(cValue) + '_rd_'+ sample.name + '_percentage_' + str(round(percMeasured, 4)) + '_variation_' + str(maskNum) + '.png', bbox_inches=extent)
            else:
                plt.savefig(dir_TrainingResultsImages + 'rand_c_' + str(cValue) + '_rd_'+ sample.name + '_percentage_' + str(round(percMeasured, 4)) + '_variation_' + str(maskNum) + '.png', bbox_inches=extent)
            plt.close()
            ...
            extent = ax.get_window_extent().transformed(fig.dpi_scale_trans.inverted())
            if incrementalFlag:
                plt.savefig(dir_TrainingResultsImages + 'incr_c_' + str(cValue) + '_measured_'+ sample.name + '_percentage_' + str(round(percMeasured, 4)) + '_variation_' + str(maskNum) + '.png', bbox_inches=extent)
            else:
                plt.savefig(dir_TrainingResultsImages + 'rand_c_' + str(cValue) + '_measured_'+ sample.name + '_percentage_' + str(round(percMeasured, 4)) + '_variation_' + str(maskNum) + '.png', bbox_inches=extent)
            plt.close()

    return dataset, RDValues, percValues
...
def initTrain(sortedTrainingSampleFolders):
    ...
    for trainingSampleFolder in tqdm(sortedTrainingSampleFolders, desc = 'Training Samples', ascii=True):
        ...
        for maskNum in tqdm(range(0,numMasks), desc = 'Masks', leave=False, ascii=True):
            for cNum in tqdm(range(0, len(cValues)), desc = 'C Values', leave=False, ascii=True):
                ...
                #If training masks should be generated incrementally (based on the RD)
                if incrementalTrainingMasks: dataset, RDValues, percValues = generateDataset(dataset, RDValues, percValues, sample, avgGroundTruthImage, percToScan, massRanges, dir_TrainingResultsImages, dataSampleName, maskNum, cValues[cNum], imageWidth, imageHeight, True)

                #If training masks should be generated randomly (based on the RD)
                if not incrementalTrainingMasks: dataset, RDValues, percValues = generateDataset(dataset, RDValues, percValues, sample, avgGroundTruthImage, percToScan, massRanges, dir_TrainingResultsImages, dataSampleName, maskNum, cValues[cNum], imageWidth, imageHeight, False)
                
                #Combine the results into the database and evaluation lists
                trainingDatabase[cNum] = trainingDatabase[cNum]+dataset
                RDPSNR_trainingResults[cNum].append(RDValues)
                perc_trainingResults[cNum].append(percValues)
    ...
    for cNum in tqdm(range(0, len(cValues)), desc = 'Pool Setup', leave=False, ascii=True):
        ...
        percents, trainingPSNR_mean = percResults(RDPSNR_trainingResults[cNum], perc_trainingResults[cNum], precision)
        
        #Determine the area of the PSNR training curve
        areaUnderCurveList.append(np.trapz(trainingPSNR_mean, percents))
        
        #Save and visualize the averaged curve for the givben c value
    ...
    #If the best c should be determined based on the determined curves (only works with the iterative, or combined mask approach
    if bestCFromTrainingData:
        #Select the c value and corresponding model that minimizes the summed total distortion across the samples
        bestIndex = np.argmax(areaUnderCurveList)
    ...
    #Save the basic database and samples
    if scanMethod == 'pointwise':
        pickle.dump(trainingSamples, open(dir_TrainingResults + 'trainingSamples-pointwise.p', 'wb'))
        pickle.dump(trainingDatabase, open(dir_TrainingResults + 'trainingDatabase-pointwise.p', 'wb'))
    elif scanMethod == 'linewise':
        pickle.dump(trainingSamples, open(dir_TrainingResults + 'trainingSamples-linewise.p', 'wb'))
        pickle.dump(trainingDatabase, open(dir_TrainingResults + 'trainingDatabase-linewise.p', 'wb'))
...
def trainModel(trainingDatabase, cValues):

    #Find a SLADS model for each of the c values
    trainingModels = []
   
    #If the best c was determined from the training PSNR curves, load and use only that value
    if bestCFromTrainingData: 
    ...
    for cNum in tqdm(range(0,len(cValues)), desc = 'c Values', leave = True, ascii=True): #For each of the proposed c values
        ...
        for trainingSample in trainingDatabase[cNum]:
            
            #Extract data of interest from the database
            dataSampleName = trainingSample.name
            maskNum = trainingSample.maskNum
            measurementPerc = trainingSample.measurementPerc
            cValue = trainingSample.cValue
            mask = trainingSample.maskObject.mask
            reconImage = trainingSample.reconImage
            measuredImage = trainingSample.measuredImage
            RDImage = trainingSample.RDImage
            maskObject = trainingSample.maskObject
            avgGroundTruthImage = trainingSample.avgGroundTruthImage
    
            #Compute PSNR between reconstruction and the ground truth
            reconPSNR = compare_psnr(avgGroundTruthImage, reconImage, data_range=reconImage.max() - reconImage.min())

            #Compute features of the reconstruction
            featureImage, featurePositions = featureExtractor(maskObject, reconImage, measuredImage, features)
            ...
            inputTensor, padding = makeTensor(inputImage)
            outputTensor, padding = makeTensor(outputImage)

            inputTensors.append(inputTensor)
            outputTensors.append(outputTensor)
            imagesPadding.append(padding)
        
        #Randomly shuffle the input/output tensors
        #temp = list(zip(inputTensors, outputTensors))
        #random.shuffle(temp)
        #inputTensors, outputTensors = zip(*temp)

        #Determine the number of samples to be used for training, rest are used for validation
        
        numTraining = (int(len(trainingSamples)*trainingSplit)*(len(measurementPercs)+1))

        #If there is no validation set, then use the training data for visualization/validation/early-cutoff values
        if numTraining == len(trainingDatabase[cNum]):
            inputTrainingTensors = inputTensors
            outputTrainingTensors = outputTensors
            trainingPadding = imagesPadding
        ...
        try:
            ...
            if modelDef == 'rbdn': model = rbdn(numStartFilters, inputTrainingTensors[0].shape[3])
        except:
            if modelDef == 'cnn': model = cnn(numStartFilters, 1)
            if modelDef == 'unet': model = unet(numStartFilters, 1)
            if modelDef == 'rbdn': model = rbdn(numStartFilters, 1)
        ...
        psnrList, currentValidationPSNR, ERDImages = epochCalculate(model, inputValidationTensors, outputValidationTensors, validationPadding, totalValidationPSNR)
        ...
        if trainingProgressionVisuals:
            epochDisplay(0, trainingDatabase, inputValidationTensors, outputValidationTensors, validationPadding, totalValidationPSNR, totalTrainingLosses, totalValidationLosses, cValues[cNum], cNum, numTraining, patience, maxPatience, bestPSNR, bestEpoch, psnrList, currentValidationPSNR, ERDImages)
        
        #Train the network
        for epoch in tqdm(range(1,numEpochs+1), desc='Epoch', leave=False, ascii=True):
            epochTrainingLosses = []
            epochTrainingMSEs = []
            ...
            if ((epoch > minimumEpochs) and (currentValidationPSNR > bestPSNR)) or (epoch == minimumEpochs): 
                model.save(dir_TrainingModelResults+'model_cValue_'+str(cValues[cNum]))
                patience = 0
                ...
                if trainingProgressionVisuals:
                    epochDisplay(epoch, trainingDatabase, inputValidationTensors, outputValidationTensors, validationPadding, totalValidationPSNR, totalTrainingLosses, totalValidationLosses, cValues[cNum], cNum, numTraining, patience, maxPatience, bestPSNR, bestEpoch, psnrList, currentValidationPSNR, ERDImages)
...
def findBestC(trainingSamples):
    ...
    for cNum in tqdm(range(0, len(cValues)), desc = 'c Values', position=0, leave=True, ascii=True):
        ...
        #Load model for the given c value
        model = tf.keras.models.load_model(dir_TrainingModelResults+'model_cValue_'+str(cValues[cNum]))
        
        areaUnderCurve = 0
        for sampleNum in tqdm(range(0, len(trainingSamples)), desc = 'Train/Val Samples', leave=False, ascii=True):
            result = runSLADS(trainingSamples, model, stopPerc, sampleNum, simulationFlag=True, trainPlotFlag=True, animationFlag=False, tqdmHide=False, bestCFlag=True)
            areaResult = result.complete(0)
